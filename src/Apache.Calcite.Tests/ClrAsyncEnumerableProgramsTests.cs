...
namespace Apache.Calcite.Tests
{
    ...
    /// <summary>
    /// Runs a query through <c>ClrAsyncEnumerablePrograms.Standard()</c>, which is the program this
    /// convention ships and the one a caller driving the planner reaches for.
    /// </summary>
    /// <remarks>
    ...
    /// shipped one was the only configuration nothing measured, and three whole shapes could not plan
    /// through it while their own suites were green: AVG, every DISTINCT aggregate, and every OVER window.
    /// Each needs a logical rewrite that belongs to no convention, and <c>Programs.ofRules</c> had cleared
    /// Calcite's rules off the planner along with it.
    ///
    ...
    public class ClrAsyncEnumerableProgramsTests
    {
        ...
        /// <returns></returns>
        /// <remarks>
        /// One <c>transform</c>, because <c>Standard()</c> is one <c>Program</c> as <c>Programs.standard</c>
        /// is, and the traits are the logical root's own rather than an empty set — an empty one asks for no
        ...
        static async Task<List<object[]>> Run(string sql)
        {
            ...
            var config = Frameworks.newConfigBuilder()
                ...
                .programs(ClrAsyncEnumerablePrograms.Standard())
                .build();

            var planner = Frameworks.getPlanner(config);
            var logical = planner.rel(planner.validate(planner.parse(sql))).project();

            var traits = ClrAsyncEnumerablePrograms.DesiredRootTraitSet(logical.getTraitSet());
