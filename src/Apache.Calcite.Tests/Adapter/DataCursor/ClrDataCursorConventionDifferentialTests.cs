...
namespace Apache.Calcite.Extensions.Adapter.DataCursor.Tests
{
    ...
    /// than a failure.
    /// </remarks>
    public class ClrDataCursorConventionDifferentialTests
    {
        ...
        /// </remarks>
        static ClrDataCursorConventionDifferentialTests()
        {
        ...
        static List<string> Run(string sql, bool clr, bool topDown = false, bool planOnly = false, bool sortedAggregate = false, bool batchNestedLoopJoin = false, bool limitSort = false, bool markJoin = false, bool excludeHashJoin = false, bool excludeMergeJoin = false, bool interpreter = false, RelOptRule[]? add = null, RelOptRule[]? remove = null)
        {
            ...
            (RelNode Physical, java.util.Map Parameters, DataContext Context) Plan()
            {
                ...
                var convention = clr ? (Convention)ClrDataCursorConvention.Instance : EnumerableConvention.INSTANCE;
            ...
            if (physical is ClrDataCursorRel)
                return CursorRows(() => { var (p, m, c) = Plan(); return ((ClrDataCursorRel)p, m, c); });
        ...
        /// again and again is <c>ClrDataCursorReadModeTests</c>' to hold.
        /// </remarks>
        static List<string> CursorRows(Func<(ClrDataCursorRel Node, java.util.Map Parameters, DataContext Context)> plan)
        {
            static (ClrDataCursorFactory Factory, DataContext Context) Open(Func<(ClrDataCursorRel Node, java.util.Map Parameters, DataContext Context)> plan)
            {
                var (node, parameters, context) = plan();
                var implementor = new ClrDataCursorRelImplementor(node.getCluster().getRexBuilder(), parameters);
        ...
        public void ShouldPlanMatchRecognizeUnderAConverter() =>
            PlanOf("SELECT * FROM \"HR\".\"emps\" MATCH_RECOGNIZE (ORDER BY \"empid\" MEASURES STRT.\"empid\" AS \"s\" PATTERN (STRT UP) DEFINE UP AS UP.\"salary\" > PREV(UP.\"salary\")) AS T", true)
                .Should().StartWith("EnumerableToClrDataCursorConverter");
        ...
        public void ShouldAgreeOnAScalarRowLimit() =>
            SameThrough("ClrDataCursorLimit", "SELECT \"N\" FROM \"SCALARS\" ORDER BY \"N\" OFFSET 1 ROWS FETCH NEXT 2 ROWS ONLY",
        ...
        public void ShouldAgreeOnAScalarRowMergeUnion() =>
            // this convention's sort over its union ties the merge union under a converter, and which of two
            // tied plans the planner keeps is the order the rules fired in, so both of ours come out too
            SameThrough("ClrEnumerableMergeUnion", "SELECT \"N\" FROM \"SCALARS\" UNION SELECT \"N\" FROM \"SCALARS\" ORDER BY 1",
                remove: [EnumerableRules.ENUMERABLE_MERGE_UNION_RULE, EnumerableRules.ENUMERABLE_UNION_RULE, EnumerableRules.ENUMERABLE_SORT_RULE, ClrDataCursorRules.ClrDataCursorUnionRule, ClrDataCursorRules.ClrDataCursorSortRule]);
