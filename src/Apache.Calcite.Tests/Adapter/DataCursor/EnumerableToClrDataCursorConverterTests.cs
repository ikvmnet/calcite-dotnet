...
namespace Apache.Calcite.Extensions.Adapter.DataCursor.Tests
{
    ...
    /// </remarks>
    public class EnumerableToClrDataCursorConverterTests
    {
        ...
        static List<object[]> Run(string sql, Convention root)
        {
            ...
            if (physical is ClrDataCursorRel clr)
            {
                var factory = new ClrDataCursorRelImplementor(clr.getCluster().getRexBuilder(), parameters).ImplementRoot(clr, ClrEnumerablePrefer.Array);
        ...
        {
            var rows = Run("SELECT \"ID\", \"NAME\" FROM \"PEOPLE\" WHERE \"ID\" > 1", ClrDataCursorConvention.Instance);
        ...
        static List<object[]> RunAcrossConverter(string sql)
        {
            ...
            rules.add(Apache.Calcite.Extensions.Adapter.DataCursor.EnumerableToClrDataCursorConverterRule.Create());
            ...
            var planner = Frameworks.getPlanner(config);
            var logical = planner.rel(planner.validate(planner.parse(sql))).project();
            var chosen = planner.transform(0, planner.getEmptyTraitSet().replace(ClrDataCursorConvention.Instance), logical);
            var physical = planner.transform(1, chosen.getTraitSet(), chosen);

            var rows = new List<object[]>();
            var factory = new ClrDataCursorRelImplementor(physical.getCluster().getRexBuilder(), new java.util.HashMap()).ImplementRoot((ClrDataCursorRel)physical, ClrEnumerablePrefer.Array);
        ...
        /// never produces a <see cref="ClrDataCursorProject"/> at all. Two rules of ours and no more is what
        ...
        /// <see cref="ClrDataCursorRules.CalcRules"/> carries a rule that names ours.</para>
        /// </remarks>
        static (string Plan, List<object[]>? Rows, string? Error) PlanResidualOverConverter(string sql, bool ourCalcPass)
        {
            ...
            rules.add(ClrDataCursorRules.ClrDataCursorProjectRule);
            rules.add(ClrDataCursorRules.EnumerableToClrDataCursorConverterRule);
            ...
            var physical = planner.transform(0, planner.getEmptyTraitSet().replace(ClrDataCursorConvention.Instance), logical);
        ...
        public void ShouldRewriteAResidualProjectOverAConverter()
        {
            ...
            plan.Should().Contain("ClrDataCursorCalc", "the residual is a calc: " + plan);
            plan.Should().NotContain("ClrDataCursorProject", "nothing unimplementable is left: " + plan);
            plan.Should().Contain("EnumerableToClrDataCursorConverter", "the converter is still there: " + plan);
        ...
        public void ShouldRefuseAResidualProjectWithoutTheCalcPass()
        {
            ...
            plan.Should().Contain("ClrDataCursorProject", "the projection survives Calcite's calc pass: " + plan);

            rows.Should().BeNull();
            error.Should().Contain("Unable to implement ClrDataCursorProject");
        }
