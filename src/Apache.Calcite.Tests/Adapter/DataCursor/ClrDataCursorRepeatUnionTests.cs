...
namespace Apache.Calcite.Extensions.Adapter.DataCursor.Tests
{
    ...
    /// <summary>
    /// What <c>ClrDataCursorDefaults.RepeatUnion</c> and its awaiting twin do, against what
    ...
    /// </remarks>
    public class ClrDataCursorRepeatUnionTests
    {

        /// <summary>
        /// A cursor over rows in hand that records its disposal.
        /// </summary>
        sealed class RowsCursor(IReadOnlyList<int> rows, Action? onDispose = null) : ClrDataCursor<int>
        {
        ...
        sealed class Rounds
        {
            ...
            public ClrDataCursor<int> Open()
            {
                var round = Evaluations++;
                return new RowsCursor(round == 0 ? [100] : []);
            }

            public async ValueTask<ClrDataCursor<int>> OpenAsync(CancellationToken cancellationToken)
            {
                await Task.Yield();
                return Open();
            }
        ...
        sealed class Empty
        {
            ...
            public ValueTask<ClrDataCursor<int>> OpenAsync(CancellationToken cancellationToken)
            {
                return new ValueTask<ClrDataCursor<int>>(Open());
            }

        }

        static List<int> ReadAll(ClrDataCursor<int> cursor)
        {
        ...
        static async Task<List<int>> ReadAllAsync(ClrDataCursor<int> cursor)
        {
            var rows = new List<int>();
        ...
        public void ShouldNotRunAnExtraRoundWhereTheFirstRoundProducedRows()
        {
            ...
            var rows = ReadAll(ClrDataCursorDefaults.RepeatUnion(new RowsCursor([1]), rounds.Open, rounds.OpenAsync, -1, true, null, null));
        ...
        public async Task ShouldNotRunAnExtraRoundWhereTheFirstRoundProducedRowsAsync()
        {
            var rounds = new Rounds();
            var openedAsync = 0;
            var cursor = await ClrDataCursorDefaults.RepeatUnionAsync(
                new ValueTask<ClrDataCursor<int>>(new RowsCursor([1])),
                rounds.Open,
        ...
        public void ShouldEvaluateAnEmptyIterativePartTwiceWhereTheSeedEmittedARow()
        {
            ...
            ReadAll(ClrDataCursorDefaults.RepeatUnion(new RowsCursor([1]), empty.Open, empty.OpenAsync, -1, true, null, null)).Should().Equal(1);
        ...
        public void ShouldEvaluateAnEmptyIterativePartOnceWhereTheSeedWasEmpty()
        {
            ...
            ReadAll(ClrDataCursorDefaults.RepeatUnion(new RowsCursor([]), empty.Open, empty.OpenAsync, -1, true, null, null)).Should().BeEmpty();
        ...
        {
            var rounds = new Rounds();
            var rows = ReadAll(ClrDataCursorDefaults.RepeatUnion(new RowsCursor([1]), rounds.Open, rounds.OpenAsync, 0, true, null, null));
        ...
        {
            var rounds = new Rounds();
            var cursor = ClrDataCursorDefaults.RepeatUnion(new RowsCursor([1, 2]), rounds.Open, rounds.OpenAsync, -1, true, null, null);
        ...
        public void ShouldRunTheCleanUpBeforeDisposingTheCursors()
        {
            ...
            using (var cursor = ClrDataCursorDefaults.RepeatUnion(new RowsCursor([1, 2], () => order.Add("seed disposed")), empty.Open, empty.OpenAsync, -1, true, null, () => order.Add("clean up")))
                cursor.Read().Should().BeTrue();
        ...
        {
            var empty = new Empty();
            var cursor = ClrDataCursorDefaults.RepeatUnion(new RowsCursor([1]), empty.Open, empty.OpenAsync, -1, true, null, null);
