...
namespace Apache.Calcite.Extensions.Adapter.DataCursor.Tests
{
    ...
    /// </remarks>
    public class ClrDataCursorRulesTests
    {

        /// <summary>
        /// Initializes the static instance.
        /// </summary>
        static ClrDataCursorRulesTests()
        {
        ...
        static async Task<List<object[]>> Run(string sql)
        {
            ...
            var traits = logical.getTraitSet().replace(ClrDataCursorConvention.Instance).simplify();
            var physical = (ClrDataCursorRel)planner.transform(0, traits, logical);

            var parameters = new java.util.HashMap();
            var factory = new ClrDataCursorRelImplementor(physical.getCluster().getRexBuilder(), parameters).ImplementRoot(physical, ClrEnumerablePrefer.Array);
