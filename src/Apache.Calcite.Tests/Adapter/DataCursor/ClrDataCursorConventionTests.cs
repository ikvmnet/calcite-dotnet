...
namespace Apache.Calcite.Extensions.Adapter.DataCursor.Tests
{
    ...
    /// </summary>
    public class ClrDataCursorConventionTests
    {
        ...
        /// </remarks>
        static ClrDataCursorConventionTests()
        {
        ...
        /// <returns></returns>
        static (ClrDataCursorRel Plan, SchemaPlus Schema) Plan(string sql)
        {
            ...
            var config = Frameworks.newConfigBuilder()
                ...
                .programs(
                    Programs.sequence(
                        new AddRulesProgram([.. ClrEnumerableRules.Rules(), .. ClrDataCursorRules.Rules()]),
                        Programs.standard(),
            ...
            var traitSet = logical.getTraitSet().replace(ClrDataCursorConvention.Instance).simplify();
            var physical = (ClrDataCursorRel)planner.transform(0, traitSet, logical);
        ...
        static List<object[]> Run(string sql)
        {
            ...
            try
            {
                factory = new ClrDataCursorRelImplementor(physical.getCluster().getRexBuilder(), new java.util.HashMap()).ImplementRoot(physical, ClrEnumerablePrefer.Array);
            }
            catch (Exception e)
            {
        ...
        /// </summary>
        static List<object[]> Rows(ClrDataCursorFactory factory, DataContext context)
        {
        ...
        public void ShouldNameThePlanWhenANodeCannotImplementItself()
        {
            ...
            var project = ClrDataCursorProject.Create(physical, identity, physical.getRowType());

            var act = () => new ClrDataCursorRelImplementor(project.getCluster().getRexBuilder(), new java.util.HashMap()).ImplementRoot(project, ClrEnumerablePrefer.Array);

            act.Should().Throw<java.lang.IllegalStateException>()
                .WithMessage("Unable to implement ClrDataCursorProject*")
        ...
        public void ShouldCombineTwoQueries()
        {
            ...
            var physical = (ClrDataCursorRel)Programs
