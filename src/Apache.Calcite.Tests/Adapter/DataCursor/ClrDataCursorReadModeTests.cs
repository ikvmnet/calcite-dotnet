...
namespace Apache.Calcite.Extensions.Adapter.DataCursor.Tests
{
    ...
    /// <see cref="ClrDataCursor.Read"/>, opened with await and read with
    /// <see cref="ClrDataCursor.ReadAsync"/>, and each open read with the other advance, including
    ...
    /// </remarks>
    public class ClrDataCursorReadModeTests
    {

        static ClrDataCursorReadModeTests()
        {
        ...
        /// </summary>
        static (ClrDataCursorFactory Factory, DataContext Context) Cursor(string sql)
        {
            var rootSchema = Schema();
            var physical = Plan(sql, rootSchema, ClrDataCursorConvention.Instance);

            physical.Should().BeAssignableTo<ClrDataCursorRel>("'{0}' was asked to end in the cursor convention", sql);

            var parameters = new java.util.HashMap();
            var implementor = new ClrDataCursorRelImplementor(physical.getCluster().getRexBuilder(), parameters);

            return (implementor.ImplementRoot((ClrDataCursorRel)physical, ClrEnumerablePrefer.Array), new TestDataContext(rootSchema, parameters));
        }
        ...
        /// <summary>
        /// Opens synchronously and reads with <see cref="ClrDataCursor.Read"/>.
        /// </summary>
        static List<string> OpenAndRead(ClrDataCursorFactory factory, DataContext context)
        {
            var rows = new List<string>();
        ...
        /// <summary>
        /// Opens with await and reads with <see cref="ClrDataCursor.ReadAsync"/>.
        /// </summary>
        static async Task<List<string>> OpenAsyncAndReadAsync(ClrDataCursorFactory factory, DataContext context)
        {
            var rows = new List<string>();
        ...
        /// </summary>
        static async Task<List<string>> OpenAndAlternate(ClrDataCursorFactory factory, DataContext context)
        {
            var rows = new List<string>();
        ...
        /// <summary>
        /// Opens with await and reads with <see cref="ClrDataCursor.Read"/>.
        /// </summary>
        static async Task<List<string>> OpenAsyncAndRead(ClrDataCursorFactory factory, DataContext context)
        {
            var rows = new List<string>();
        ...
        public void ShouldCarryACursorPlanUnderACalciteNode()
        {
            ...
            RelOptUtil.toString(physical).Should().Contain("ClrDataCursorToEnumerableConverter")
                .And.Contain("ClrDataCursorTableScan");
