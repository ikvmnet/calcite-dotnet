...
namespace Apache.Calcite.Data.Types.Tests
{
    ...
    public class ClrTypeRegistryTests
    {
        ...
        RelDataType Interval(TimeUnit start, TimeUnit? end)
        {
            var qualifier = new SqlIntervalQualifier(start, end, SqlParserPos.ZERO);
            return _types.createTypeWithNullability(_types.createSqlIntervalType(qualifier), true);
        }

        [Theory]
        [InlineData(nameof(TimeUnit.YEAR), null, nameof(SqlTypeName.INTERVAL_YEAR), typeof(int))]
        [InlineData(nameof(TimeUnit.YEAR), nameof(TimeUnit.MONTH), nameof(SqlTypeName.INTERVAL_YEAR_MONTH), typeof(int))]
        [InlineData(nameof(TimeUnit.MONTH), null, nameof(SqlTypeName.INTERVAL_MONTH), typeof(int))]
        [InlineData(nameof(TimeUnit.DAY), null, nameof(SqlTypeName.INTERVAL_DAY), typeof(long))]
        [InlineData(nameof(TimeUnit.HOUR), nameof(TimeUnit.SECOND), nameof(SqlTypeName.INTERVAL_HOUR_SECOND), typeof(long))]
        [InlineData(nameof(TimeUnit.SECOND), null, nameof(SqlTypeName.INTERVAL_SECOND), typeof(long))]
        public void An_interval_should_be_read_as_the_count_it_holds(string start, string? end, string sqlTypeName, Type expected)
        {
            // months in an Integer for the year-month intervals, milliseconds in a Long for the day-time
            // ones, which is what getJavaClass says and the same two classes a DATE and a TIMESTAMP use
            var relType = Interval(TimeUnit.valueOf(start), end is null ? null : TimeUnit.valueOf(end));

            Assert.Equal(sqlTypeName, relType.getSqlTypeName().name());
            Assert.Equal(expected, _registry.GetClrType(relType));
        }
        ...
        [InlineData(typeof(Guid), nameof(SqlTypeName.UUID))]
        ...
        {
            // nothing about a CHAR(36) says it holds one; Calcite's own type for a UUID is UUID, and
            // reading one out of a character column is something a caller has to ask for by name
            Assert.Equal(typeof(string), _registry.GetClrType(Sql(SqlTypeName.CHAR, 36)));
        }

        [Fact]
        public void A_CHAR_36_should_be_read_back_as_a_Guid_where_one_is_asked_for()
        {
            var mapping = _registry.RequireMapping(typeof(Guid), Sql(SqlTypeName.CHAR, 36));
            Assert.Equal(Guid.Parse("00000000-0000-0000-0000-000000000001"), mapping.FromCalcite("00000000-0000-0000-0000-000000000001"));
        }
        ...
        // ------------------------------------------------------------------------------------
        // A type built out of other types.
        // ------------------------------------------------------------------------------------

        RelDataType Array(RelDataType component) => _types.createTypeWithNullability(_types.createArrayType(component, -1), true);

        [Fact]
        public void An_array_should_be_read_as_an_array_of_its_component()
        {
            Assert.Equal(typeof(int[]), _registry.GetClrType(Array(Sql(SqlTypeName.INTEGER))));
            Assert.Equal(typeof(string[]), _registry.GetClrType(Array(Sql(SqlTypeName.VARCHAR))));
            Assert.Equal(typeof(int[][]), _registry.GetClrType(Array(Array(Sql(SqlTypeName.INTEGER)))));
        }

        [Fact]
        public void A_map_should_be_read_as_a_dictionary_of_its_key_and_value()
        {
            var relType = _types.createTypeWithNullability(_types.createMapType(Sql(SqlTypeName.VARCHAR), Sql(SqlTypeName.INTEGER)), true);
            Assert.Equal(typeof(Dictionary<string, int>), _registry.GetClrType(relType));
        }

        [Fact]
        public void An_array_should_convert_its_elements_through_the_component_mapping()
        {
            var relType = Array(Sql(SqlTypeName.DATE));
            var list = new java.util.ArrayList();
            list.add(java.lang.Integer.valueOf(0));

            // a DATE is a count of days, and only the component type says so
            Assert.Equal(new[] { new DateTime(1970, 1, 1, 0, 0, 0, DateTimeKind.Utc) }, Assert.IsType<DateTime[]>(_registry.FromCalcite(null, relType, list)));
        }

        [Fact]
        public void An_array_should_convert_what_it_is_written_from_through_the_component_mapping()
        {
            var relType = Array(Sql(SqlTypeName.DATE));
            var written = Assert.IsAssignableFrom<java.util.List>(_registry.ToCalcite(null, relType, new[] { new DateOnly(1970, 1, 2) }));

            Assert.Equal(1, written.size());
            Assert.Equal(java.lang.Integer.valueOf(1), written.get(0));
        }
        ...
        public void A_claim_over_a_component_should_carry_to_the_array_of_it()
        {
            ...
            var relType = Array(Sql(SqlTypeName.VARCHAR));

            Assert.Equal(typeof(Uri[]), registry.GetClrType(relType));

            var list = new java.util.ArrayList();
            list.add("https://calcite.apache.org/");
            Assert.Equal(new[] { new Uri("https://calcite.apache.org/") }, Assert.IsType<Uri[]>(registry.FromCalcite(null, relType, list)));
        }
        ...
        sealed class WrongResolver : IClrTypeResolver
        {
            ...
            {
                _mappings.Add(typeof(int), SqlTypeName.INTEGER, (v, _) => v.ToString(), (v, _) => v);
            }
