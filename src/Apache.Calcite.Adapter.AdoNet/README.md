...
Anything that cannot be pushed down runs in-process. Converters exist into all three execution conventions — `AdoToClrDataCursorConverter` for the cursor convention that `Apache.Calcite.Data` plans into, `AdoToClrEnumerableConverter` for the compiled-.NET sequence convention, and `AdoToEnumerableConverter` for Calcite's own — so the adapter works under any. The first hands the `DbDataReader` back as the plan's cursor: `ExecuteReader` or `ExecuteReaderAsync` is the open, each `Read` or `ReadAsync(token)` of the reader `Apache.Calcite.Data` gives you is an advance of the provider's reader, and the token of that call is the token the provider's `ReadAsync` is given. The second reads the same reader as a sequence, through `ReadAsync` where the plan awaits and `Read` where it does not, chosen when the plan is compiled rather than when it is planned. In every case the statement is sent when the plan is opened, which is where these conventions acquire.
