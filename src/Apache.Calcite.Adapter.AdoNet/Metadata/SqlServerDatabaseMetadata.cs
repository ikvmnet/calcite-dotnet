...
namespace Apache.Calcite.Adapter.AdoNet.Metadata
{
    ...
    /// <summary>
    /// Implements the <see cref="AdoDatabaseMetadata"/> for a <see cref="SqlConnection"/>.
    /// </summary>
    class SqlServerDatabaseMetadata : AdoInformationSchemaDatabaseMetadata<SqlConnection>
    {
        ...
        {
            // SQL provides this on the connection string
            var defaultDatabase = new SqlConnectionStringBuilder(DbDataSource.ConnectionString).InitialCatalog;
        ...
        {
            using var cnn = DbDataSource.OpenConnection() as SqlConnection;
            if (cnn is null)
                throw new NotSupportedException("SqlServerMetadataProvider only supports SqlConnection.");
