    ...
    internal readonly struct CalciteResultColumns
    {
        ...
        public static Type MapClrType(ColumnMetaData.AvaticaType type)
        {
            ...
            switch (type.name)
            {
                ...
                case "VARBINARY":
                    return typeof(byte[]);
                // Calcite's runtime representation of a UUID is an org.apache.calcite.util.UuidValue,
                // whose rep is OBJECT like every class Avatica has no name of its own for, so the rep
                // cannot say what this is and the SQL type name has to
                case "UUID":
                    return typeof(Guid);
