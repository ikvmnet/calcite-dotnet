...
| `ClrCursorFactory` | A compiled plan of the cursor convention: `Open(DataContext)` and `OpenAsync(DataContext, CancellationToken)` each hand back a `ClrCursor`, and `ElementType` says what one row is. |
| `ClrCursor` / `ClrCursor<T>` | A forward-only cursor with `Read()` and `ReadAsync(CancellationToken)` over one position, and `Current`. |
