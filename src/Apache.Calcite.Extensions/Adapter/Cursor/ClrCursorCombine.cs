...
namespace Apache.Calcite.Extensions.Adapter.Cursor
{
    ...
    /// <summary>
    /// Implementation of <see cref="Combine"/> in the <see cref="ClrCursorConvention"/> calling
    /// convention.
    /// </summary>
    /// <remarks>
    ...
    public class ClrCursorCombine : Combine, ClrCursorRel
    {
        ...
        {
            return new ClrCursorCombine(getCluster(), traitSet, inputs);
        }
        ...
        public ClrCursorResult Implement(ClrCursorRelImplementor implementor, ClrEnumerablePrefer pref)
        {
            ...
            {
                var input = (ClrCursorRel)getInputs().get(ord);
                ...
                var mapped = Expression.Call(null,
                    ClrCursorBuiltInMethod.Select.MakeGenericMethod(sourceType, typeof(java.util.Map)),
                    source,
                    selector);

                lists.add(Expression.Call(null, ClrCursorBuiltInMethod.ToJavaList.MakeGenericMethod(typeof(java.util.Map)), mapped));
            }
            ...
            return implementor.Result(physType,
                Expression.Call(null, ClrCursorBuiltInMethod.FromJavaList.MakeGenericMethod(typeof(object[])), combined));
        }
        ...
        public ClrCursorAsyncResult ImplementAsync(ClrCursorRelImplementor implementor, ClrEnumerablePrefer pref)
        {
            ...
            for (int ord = 0; ord < getInputs().size(); ord++)
            {
                ...
                var mapped = ClrCursorBuiltInMethod.CallAsync(implementor, ClrCursorBuiltInMethod.SelectAsync.MakeGenericMethod(sourceType, typeof(java.util.Map)),
                    source,
                    selector);
