...
namespace Apache.Calcite.Extensions.Adapter.Cursor
{
    ...
    /// <summary>
    /// Relational operator that reads the result of a <see cref="ClrCursorConvention"/> sub-plan as a
    /// <see cref="ClrEnumerableConvention"/> one.
    /// </summary>
    /// <remarks>
    /// The other direction of <see cref="ClrEnumerableToClrCursorConverter"/>, and spliced the same way.
    ...
    /// </remarks>
    public class ClrCursorToClrEnumerableConverter : ConverterImpl, ClrEnumerableRel
    {
        ...
        public ClrCursorToClrEnumerableConverter(RelOptCluster cluster, RelTraitSet traits, RelNode input) :
        ...
        {
            return new ClrCursorToClrEnumerableConverter(getCluster(), traitSet, (RelNode)sole(inputs));
        }
        ...
        public ClrEnumerableResult Implement(ClrEnumerableRelImplementor implementor, ClrEnumerablePrefer pref)
        {
            ...
            var result = cursor.VisitChild(null, 0, (ClrCursorRel)getInput(), pref);

            return implementor.Result(result.PhysType,
                Expression.Call(null, ClrCursorBuiltInMethod.AsEnumerable.MakeGenericMethod(result.PhysType.RowType), cursor.Opener(result)));
        }
        ...
        {
            var cursor = Cursor(implementor);
            var result = cursor.VisitChildAsync(null, 0, (ClrCursorRel)getInput(), pref);

            return implementor.ResultAsync(result.PhysType,
                ClrBuiltInMethod.CallAsync(ClrCursorBuiltInMethod.AsAsyncEnumerable.MakeGenericMethod(result.PhysType.RowType), cursor.OpenerAsync(result)));
        }
        ...
        /// </remarks>
        static ClrCursorRelImplementor Cursor(ClrEnumerableRelImplementor implementor)
        {
            var cursor = new ClrCursorRelImplementor(implementor.RexBuilder, implementor.Map, implementor.Root, Expression.Parameter(typeof(CancellationToken), "cancellationToken"), implementor.Translator);
