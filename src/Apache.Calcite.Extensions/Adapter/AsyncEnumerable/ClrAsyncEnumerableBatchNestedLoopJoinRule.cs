...
namespace Apache.Calcite.Extensions.Adapter.AsyncEnumerable
{
    ...
    /// <see cref="ClrAsyncEnumerableBatchNestedLoopJoin"/>.
    /// </summary>
    /// <remarks>
    ...
    /// </remarks>
    public class ClrAsyncEnumerableBatchNestedLoopJoinRule : RelRule
    {

        /// <summary>
        /// Creates a <see cref="ClrAsyncEnumerableBatchNestedLoopJoinRule"/>, with Calcite's default batch size.
        /// </summary>
        /// <returns></returns>
        public static ClrAsyncEnumerableBatchNestedLoopJoinRule Create()
        {
            return Create(org.apache.calcite.adapter.enumerable.EnumerableBatchNestedLoopJoinRule.Config.DEFAULT.batchSize());
        }

        /// <summary>
        /// Creates a <see cref="ClrAsyncEnumerableBatchNestedLoopJoinRule"/>.
        /// </summary>
        /// <param name="batchSize">How many left rows one pass of the right input serves.</param>
        /// <returns></returns>
        public static ClrAsyncEnumerableBatchNestedLoopJoinRule Create(int batchSize)
        {
            var config = org.apache.calcite.adapter.enumerable.EnumerableBatchNestedLoopJoinRule.Config.DEFAULT
                .withBatchSize(batchSize)
                .withDescription($"ClrAsyncEnumerableBatchNestedLoopJoinRule({batchSize})");

            return new ClrAsyncEnumerableBatchNestedLoopJoinRule((RelRule.Config)config, batchSize);
        }
        ...
        public ClrAsyncEnumerableBatchNestedLoopJoinRule(RelRule.Config config, int batchSize) :
            base(config)
        {
        ...
        public override void onMatch(RelOptRuleCall call)
        {
            ...
            call.transformTo(
                ClrAsyncEnumerableBatchNestedLoopJoin.Create(
                    convert(call.getPlanner(), join.getLeft(), join.getLeft().getTraitSet().replace(ClrAsyncEnumerableConvention.Instance)),
                    convert(call.getPlanner(), right, right.getTraitSet().replace(ClrAsyncEnumerableConvention.Instance)),
                    join.getCondition(),
