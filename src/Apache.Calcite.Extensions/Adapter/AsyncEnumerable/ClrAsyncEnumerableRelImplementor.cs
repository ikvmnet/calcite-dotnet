...
namespace Apache.Calcite.Extensions.Adapter.AsyncEnumerable
{
    ...
    /// <summary>
    /// Turns a tree of <see cref="ClrAsyncEnumerableRel"/> into the expression that runs it.
    /// </summary>
    /// <remarks>
    /// <see cref="ClrEnumerableRelImplementor"/>, member for member, for a plan whose sequences are
    /// <see cref="IAsyncEnumerable{T}"/>. One instance implements one plan; a node reaches its inputs through
    /// <see cref="VisitChild"/> and returns a <see cref="ClrAsyncEnumerableResult"/>.
    ///
    /// <para>Everything to do with a <em>row</em> is the other implementor's code unchanged, because a row
    ...
    public class ClrAsyncEnumerableRelImplementor
    {
        ...
        public ClrAsyncEnumerableResult VisitChild(ClrAsyncEnumerableRel? parent, int ordinal, ClrAsyncEnumerableRel child, ClrEnumerablePrefer prefer)
        {
        ...
        /// <see cref="LambdaExpression.Compile()"/> gives a
        ...
        /// inner exception.
        /// </exception>
        public LambdaExpression ImplementRoot(ClrAsyncEnumerableRel rootRel, ClrEnumerablePrefer prefer)
        {
            ...
            if (prefer == ClrEnumerablePrefer.Array
                ...
                result = new ClrAsyncEnumerableResult(
                    ClrAsyncBuiltInMethod.Call(ClrAsyncBuiltInMethod.Slice0.MakeGenericMethod(typeof(object)), result.Expression),
                    result.PhysType,
                    JavaRowFormat.SCALAR);

            // IAsyncEnumerable<object>, named for the same reason a node's row type is named in
            // RequireRowType. The conversion is by variance and cannot fail: a row is never a value type,
            // because ClrPhysTypeImpl boxes what the type factory answers and RequireRowType holds every node
            // to that.
            return Expression.Lambda<Func<DataContext, IAsyncEnumerable<object>>>(
                Expression.Convert(result.Expression, typeof(IAsyncEnumerable<object>)),
                Root);
        }
        ...
        /// <returns></returns>
        public ClrAsyncEnumerableResult Result(ClrPhysType physType, Expression expression)
        {
        ...
        /// <remarks>
        /// <c>ClrEnumerableRelImplementor.RequireRowType</c>, naming <see cref="IAsyncEnumerable{T}"/> where
        /// that names <see cref="IEnumerable{T}"/>. It refuses rather than repairing, for the reason given
        ...
        static void RequireRowType(ClrPhysType physType, Expression expression)
        {
            ...
            if (expression.Type.IsGenericType == false || expression.Type.GetGenericTypeDefinition() != typeof(IAsyncEnumerable<>))
                throw new java.lang.IllegalStateException($"{Node()} handed up a {expression.Type} where an asynchronous sequence of {expected} was wanted.");
        ...
        static string Node()
        {
            ...
            for (int i = 1; i < trace.FrameCount; i++)
            {
                ...
                if (type != null && type != typeof(ClrAsyncEnumerableRelImplementor))
                    return type.Name;
            }
