...
namespace Apache.Calcite.Extensions.Adapter.AsyncEnumerable
{
    ...
    /// <summary>
    /// Rule that converts a <see cref="LogicalJoin"/> to a <see cref="ClrAsyncEnumerableMergeJoin"/>.
    /// </summary>
    /// <remarks>
    ...
    /// </remarks>
    public class ClrAsyncEnumerableMergeJoinRule : ConverterRule
    {

        /// <summary>
        /// Creates a <see cref="ClrAsyncEnumerableMergeJoinRule"/>.
        /// </summary>
        /// <returns></returns>
        public static ClrAsyncEnumerableMergeJoinRule Create()
        {
            return (ClrAsyncEnumerableMergeJoinRule)Config.INSTANCE
                .withConversion((java.lang.Class)typeof(LogicalJoin), Convention.NONE, ClrAsyncEnumerableConvention.Instance, "ClrAsyncEnumerableMergeJoinRule")
                .withRuleFactory(new DelegateFunction<Config, ClrAsyncEnumerableMergeJoinRule>(c => new ClrAsyncEnumerableMergeJoinRule(c)))
                .toRule(typeof(ClrAsyncEnumerableMergeJoinRule));
        }
        ...
        public ClrAsyncEnumerableMergeJoinRule(Config config) :
            base(config)
        {
        ...
        public override RelNode convert(RelNode rel)
        {
            ...
            for (int ord = 0; ord < join.getInputs().size(); ord++)
            {
                ...
                var traits = input.getTraitSet().replace(ClrAsyncEnumerableConvention.Instance);
            ...
            var traitSet = join.getTraitSet().replace(ClrAsyncEnumerableConvention.Instance);
            ...
            return new ClrAsyncEnumerableMergeJoin(join.getCluster(), traitSet, left, right, condition, join.getVariablesSet(), join.getJoinType());
        }
