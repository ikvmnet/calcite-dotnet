...
namespace Apache.Calcite.Extensions.Adapter.AsyncEnumerable
{
    ...
    /// <summary>
    /// Implementation of <see cref="Calc"/> in the <see cref="ClrAsyncEnumerableConvention"/> calling convention.
    /// </summary>
    /// <remarks>
    /// Calcite fuses the filter and the projection into one anonymous <c>Enumerator</c>, because generated
    /// Java source is the only place it can put a custom enumerator. <see cref="ClrAsyncEnumerableDefaults.Calc"/> is that
    /// enumerator, written once as an ordinary iterator and called from the tree, so the plan is the same one
    /// pass over the input.
    /// </remarks>
    public class ClrAsyncEnumerableCalc : Calc, ClrAsyncEnumerableRel
    {

        /// <summary>
        /// Creates a <see cref="ClrAsyncEnumerableCalc"/>.
        /// </summary>
        /// <param name="input"></param>
        /// <param name="program"></param>
        /// <returns></returns>
        public static ClrAsyncEnumerableCalc Create(RelNode input, RexProgram program)
        {
            ...
            return new ClrAsyncEnumerableCalc(cluster, traitSet, input, program);
        }
        ...
        /// <param name="input"></param>
        /// <param name="program"></param>
        public ClrAsyncEnumerableCalc(RelOptCluster cluster, RelTraitSet traitSet, RelNode input, RexProgram program) :
        ...
        {
            return new ClrAsyncEnumerableCalc(getCluster(), traitSet, child, program);
        }
        ...
        public ClrAsyncEnumerableResult Implement(ClrAsyncEnumerableRelImplementor implementor, ClrEnumerablePrefer pref)
        {
            ...
            return implementor.Result(physType,
                ClrAsyncBuiltInMethod.Call(ClrAsyncBuiltInMethod.Calc.MakeGenericMethod(inputType, outputType), result.Expression, predicate, selector));
        }
