...
namespace Apache.Calcite.Extensions.Adapter.AsyncEnumerable
{
    ...
    /// <see cref="ClrAsyncEnumerableMergeUnion"/>.
    /// </summary>
    /// <remarks>
    ...
    /// </remarks>
    public class ClrAsyncEnumerableMergeUnionRule : RelRule
    {

        /// <summary>
        /// Creates a <see cref="ClrAsyncEnumerableMergeUnionRule"/>.
        /// </summary>
        /// <returns></returns>
        public static ClrAsyncEnumerableMergeUnionRule Create()
        {
            var config = org.apache.calcite.adapter.enumerable.EnumerableMergeUnionRule.Config.DEFAULT_CONFIG
                .withDescription("ClrAsyncEnumerableMergeUnionRule");

            return new ClrAsyncEnumerableMergeUnionRule(config);
        }
        ...
        public ClrAsyncEnumerableMergeUnionRule(RelRule.Config config) :
            base(config)
        {
        ...
        public override void onMatch(RelOptRuleCall call)
        {
            ...
            for (int i = 0; i < union.getInputs().size(); i++)
            {
                ...
                inputs.add(convert(call.getPlanner(), newInput, newInput.getTraitSet().replace(ClrAsyncEnumerableConvention.Instance)));
            }

            RelNode result = ClrAsyncEnumerableMergeUnion.Create(sort.getCollation(), inputs, union.all);

            // the merge union's rows are already in order, so an offset or a limit is all that is left of the
            // sort
            if (sort.offset != null || sort.fetch != null)
                result = ClrAsyncEnumerableLimit.Create(result, sort.offset, sort.fetch);
