...
namespace Apache.Calcite.Extensions.Adapter.Enumerable
{
    ...
    static class ClrEnumerableDefaults
    {
        ...
        /// <summary>
        /// Folds every row into an accumulator and returns the one row that comes out.
        /// </summary>
        /// <typeparam name="TSource"></typeparam>
        /// <typeparam name="TResult"></typeparam>
        /// <param name="source"></param>
        ...
        /// <returns></returns>
        /// <remarks>
        /// <see cref="Singleton"/> over <see cref="Aggregate"/>, which is what <c>EnumerableAggregate</c>
        /// generates as two nested calls. It is one operator here because the asynchronous side has to be:
        /// there the fold must be awaited and an expression tree cannot await, so the composition is an
        /// operator rather than a tree, and a node builds the same call in both modes.
        ///
        /// <para>Not an iterator method, deliberately. Calcite's inner call is an ordinary method returning
        /// the row, so the fold runs where the expression is evaluated rather than at the first
        /// <c>MoveNext</c>, and keeping that is what keeps the timing Calcite has.</para>
        /// </remarks>
        public static IEnumerable<TResult> SingletonAggregate<TSource, TResult>(IEnumerable<TSource> source, object seed, Function2 accumulatorAdder, Function1 resultSelector)
        {
            ArgumentNullException.ThrowIfNull(source);

            return Singleton(Aggregate<TSource, TResult>(source, seed, accumulatorAdder, resultSelector));
        }

        /// <summary>
        /// Reads every row into a Java list and returns the one row that list is.
        /// </summary>
        /// <typeparam name="TSource"></typeparam>
        /// <param name="source"></param>
        /// <returns></returns>
        /// <remarks>
        /// <see cref="Singleton"/> over <see cref="ToJavaList"/>, fused for the reason
        /// <see cref="SingletonAggregate"/> gives, and eager at the call for the same reason.
        /// </remarks>
        public static IEnumerable<java.util.List> SingletonJavaList<TSource>(IEnumerable<TSource> source)
        {
            ArgumentNullException.ThrowIfNull(source);

            return Singleton(ToJavaList(source));
        }

        /// <summary>
        /// Reads every row into a Java map and returns the one row that map is.
        /// </summary>
        /// <typeparam name="TSource"></typeparam>
        /// <param name="source"></param>
        /// <param name="keySelector"></param>
        ...
        /// <returns></returns>
        /// <remarks>
        /// <see cref="Singleton"/> over <see cref="ToJavaMap"/>, fused for the reason
        /// <see cref="SingletonAggregate"/> gives, and eager at the call for the same reason.
        /// </remarks>
        public static IEnumerable<java.util.Map> SingletonJavaMap<TSource>(IEnumerable<TSource> source, Func<TSource, object> keySelector, Func<TSource, object> valueSelector)
        {
            ArgumentNullException.ThrowIfNull(source);

            return Singleton(ToJavaMap(source, keySelector, valueSelector));
        }
        ...
        /// <returns></returns>
        /// <remarks>
        /// Fused for the reason <see cref="SingletonAggregate"/> gives: <c>EnumerableCombine</c> reads each
        /// input into a list inside the tree, and on the asynchronous side each of those reads has to be
        /// awaited. The inputs are read in order, at the call, as evaluating those nested calls did.
        ///
        /// <para>The combining itself is not this operator's business and arrives as a delegate, so which
        /// function Calcite combines with stays the node's decision.</para>
        /// </remarks>
        public static IEnumerable<TResult> CombineQueryResults<TResult>(IEnumerable<java.util.Map>[] sources, Func<java.util.List[], java.util.List> combine)
        {
