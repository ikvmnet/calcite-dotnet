...
namespace Apache.Calcite.Extensions.Adapter.Enumerable
{
    ...
    public class ClrEnumerableWindow : Window, ClrEnumerableRel
    {
        ...
        public ClrEnumerableResult ImplementAsync(ClrEnumerableRelImplementor implementor, ClrEnumerablePrefer pref)
        {
            ...
            {
                var group = (Group)groups.get(windowIdx);
                source = ImplementGroupAsync(implementor, result, group, windowIdx, physType, source, pref, translatedConstants, variables, body, out physType);
            ...
            out ClrPhysType outputPhysType)
        {
            ...
            return ClrBuiltInMethod.CallAsync(ClrBuiltInMethod.WindowAsync.MakeGenericMethod(sourceType, keyType, accType, outputType),
                source,
                (Expression?)partitionSelector ?? Expression.Constant(null, typeof(Func<,>).MakeGenericType(sourceType, keyType)),
                comparator,
                Expression.Constant(group.exclude, typeof(RexWindowExclusion)),
