...
namespace Apache.Calcite.Extensions.Adapter.DataCursor
{
    ...
    /// <summary>
    /// Implementation of <see cref="Aggregate"/> in the <see cref="ClrDataCursorConvention"/> calling
    /// convention.
    /// </summary>
    /// <remarks>
    ...
    /// aggregate is on <see cref="ClrDataCursorAggregateBase"/>, exactly as Calcite splits the two.</para>
    ///
    ...
    /// </remarks>
    public class ClrDataCursorAggregate : ClrDataCursorAggregateBase, ClrDataCursorRel
    {
        ...
        public ClrDataCursorAggregate(RelOptCluster cluster, RelTraitSet traitSet, RelNode input, ImmutableBitSet groupSet, java.util.List groupSets, java.util.List aggCalls) :
            base(cluster, traitSet, com.google.common.collect.ImmutableList.of(), input, groupSet, groupSets, aggCalls)
        {
            for (int i = 0; i < aggCalls.size(); i++)
            {
                ...
                // will be implemented against -- ClrDataCursorAggregateRule asks the cluster's
            }
        }
        ...
        {
            return new ClrDataCursorAggregate(getCluster(), traitSet, input, groupSet, groupSets, aggCalls);
        }
        ...
        public ClrDataCursorResult Implement(ClrDataCursorRelImplementor implementor, ClrEnumerablePrefer pref)
        {
            ...
            if (getGroupType() != Group.SIMPLE)
            {
                ...
                return implementor.Result(physType,
                    Expression.Call(null,
                        ClrDataCursorBuiltInMethod.GroupByMultiple.MakeGenericMethod(sourceType, keyType!, rowType),
                        result.Expression,
            ...
            if (groupCount == 0)
            {
                ...
                return implementor.Result(physType,
                    Expression.Call(null,
                        ClrDataCursorBuiltInMethod.Singleton.MakeGenericMethod(rowType),
                        Expression.Call(null,
                            ClrDataCursorBuiltInMethod.Aggregate.MakeGenericMethod(sourceType, rowType),
                            result.Expression,
            ...
            if (getAggCallList().isEmpty() && groupSet.equals(ImmutableBitSet.range(getInput().getRowType().getFieldCount())))
            {
                ...
                return implementor.Result(physType,
                    Expression.Call(null,
                        ClrDataCursorBuiltInMethod.Distinct.MakeGenericMethod(source.Type.GetGenericArguments()[0]),
                        source,
            ...
            return implementor.Result(physType,
                Expression.Call(null,
                    ClrDataCursorBuiltInMethod.GroupBy.MakeGenericMethod(sourceType, keySelector.ReturnType, rowType),
                    result.Expression,
                    keySelector,
                    Expression.Call(lambdaFactory, AccInitializer),
                    Expression.Call(lambdaFactory, AccAdder),
        ...
        public ClrDataCursorAsyncResult ImplementAsync(ClrDataCursorRelImplementor implementor, ClrEnumerablePrefer pref)
        {
            ...
            if (getGroupType() != Group.SIMPLE)
            {
                ...
                return implementor.ResultAsync(physType,
                    ClrDataCursorBuiltInMethod.CallAsync(implementor, ClrDataCursorBuiltInMethod.GroupByMultipleAsync.MakeGenericMethod(sourceType, keyType!, rowType),
                        result.Expression,
                        Expression.NewArrayInit(typeof(Func<,>).MakeGenericType(sourceType, keyType!), selectors),
                        Expression.Call(lambdaFactory, AccInitializer),
                        Expression.Call(lambdaFactory, AccAdder),
            ...
            if (groupCount == 0)
            {
                ...
                // as the pair does. See ClrDataCursorDefaults.SingletonAggregateAsync.
                return implementor.ResultAsync(physType,
                    ClrDataCursorBuiltInMethod.CallAsync(implementor, ClrDataCursorBuiltInMethod.SingletonAggregateAsync.MakeGenericMethod(sourceType, rowType),
                        result.Expression,
                        Expression.Call(Expression.Call(lambdaFactory, AccInitializer), Function0Apply),
                        Expression.Call(lambdaFactory, AccAdder),
                        Expression.Call(lambdaFactory, SingleGroupResultSelector, Function1Of(resultSelector, accType, rowType))));
            ...
            if (getAggCallList().isEmpty() && groupSet.equals(ImmutableBitSet.range(getInput().getRowType().getFieldCount())))
            {
                ...
                return implementor.ResultAsync(physType,
                    ClrDataCursorBuiltInMethod.CallAsync(implementor, ClrDataCursorBuiltInMethod.DistinctAsync.MakeGenericMethod(source.Type.GetGenericArguments()[0].GetGenericArguments()[0]),
                        source,
                        physType.Comparer() ?? Expression.Constant(null, typeof(org.apache.calcite.linq4j.function.EqualityComparer))));
            }
            ...
            return implementor.ResultAsync(physType,
                ClrDataCursorBuiltInMethod.CallAsync(implementor, ClrDataCursorBuiltInMethod.GroupByAsync.MakeGenericMethod(sourceType, keySelector.ReturnType, rowType),
                    result.Expression,
                    keySelector,
                    Expression.Call(lambdaFactory, AccInitializer),
                    Expression.Call(lambdaFactory, AccAdder),
