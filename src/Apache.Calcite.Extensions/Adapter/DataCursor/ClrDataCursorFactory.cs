...
namespace Apache.Calcite.Extensions.Adapter.DataCursor
{
    ...
    /// <summary>
    /// A plan of the <see cref="ClrDataCursorConvention"/> calling convention, ready to open against a
    /// <see cref="DataContext"/>.
    /// </summary>
    /// <remarks>
    /// What <see cref="ClrDataCursorRelImplementor.ImplementRoot"/> produces, and the counterpart of the
    ...
    /// <see cref="ClrDataCursor"/> with both <see cref="ClrDataCursor.Read"/> and
    /// <see cref="ClrDataCursor.ReadAsync"/> over one position, so a caller chooses how to open and then
    ...
    ///
    /// <para>It is the <see cref="IClrDataCursorBindable"/> a prepared statement carries: the counterpart
    ...
    /// statement.</para>
    /// </remarks>
    public sealed class ClrDataCursorFactory : IClrDataCursorBindable
    {

        readonly Expression<Func<DataContext, ClrDataCursor>> open;
        readonly Expression<Func<DataContext, CancellationToken, ValueTask<ClrDataCursor>>> openAsync;
        readonly Type elementType;

        Func<DataContext, ClrDataCursor>? compiledOpen;
        Func<DataContext, CancellationToken, ValueTask<ClrDataCursor>>? compiledOpenAsync;
        ...
        /// <remarks>
        /// Internal, so that <see cref="ClrDataCursorRelImplementor.ImplementRoot"/> is the only way of
        /// making one: that is where the two trees are required to be two opens of one plan.
        /// </remarks>
        internal ClrDataCursorFactory(
            Expression<Func<DataContext, ClrDataCursor>> open,
            Expression<Func<DataContext, CancellationToken, ValueTask<ClrDataCursor>>> openAsync,
            Type elementType)
        {
        ...
        /// </remarks>
        public Expression<Func<DataContext, ClrDataCursor>> OpenExpression => open;

        /// <summary>
        /// Gets the plan as an open that awaits its acquisition, before it is compiled.
        /// </summary>
        public Expression<Func<DataContext, CancellationToken, ValueTask<ClrDataCursor>>> OpenAsyncExpression => openAsync;
        ...
        /// <returns>The cursor, positioned before the first row.</returns>
        public ValueTask<ClrDataCursor> OpenAsync(DataContext root, CancellationToken cancellationToken)
        {
            ArgumentNullException.ThrowIfNull(root);
