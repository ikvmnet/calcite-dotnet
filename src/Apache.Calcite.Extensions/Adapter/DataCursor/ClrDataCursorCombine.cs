...
namespace Apache.Calcite.Extensions.Adapter.DataCursor
{
    ...
    /// <summary>
    /// Implementation of <see cref="Combine"/> in the <see cref="ClrDataCursorConvention"/> calling
    /// convention.
    /// </summary>
    /// <remarks>
    ...
    /// </remarks>
    public class ClrDataCursorCombine : Combine, ClrDataCursorRel
    {
        ...
        public ClrDataCursorCombine(RelOptCluster cluster, RelTraitSet traitSet, java.util.List inputs) :
        ...
        {
            return new ClrDataCursorCombine(getCluster(), traitSet, inputs);
        }
        ...
        public ClrDataCursorResult Implement(ClrDataCursorRelImplementor implementor, ClrEnumerablePrefer pref)
        {
            ...
            {
                var input = (ClrDataCursorRel)getInputs().get(ord);
                ...
                var mapped = Expression.Call(null,
                    ClrDataCursorBuiltInMethod.Select.MakeGenericMethod(sourceType, typeof(java.util.Map)),
                    source,
                    selector);

                lists.add(Expression.Call(null, ClrDataCursorBuiltInMethod.ToJavaList.MakeGenericMethod(typeof(java.util.Map)), mapped));
            }
            ...
            return implementor.Result(physType,
                Expression.Call(null, ClrDataCursorBuiltInMethod.FromJavaList.MakeGenericMethod(typeof(object[])), combined));
        }
        ...
        public ClrDataCursorAsyncResult ImplementAsync(ClrDataCursorRelImplementor implementor, ClrEnumerablePrefer pref)
        {
            ...
            for (int ord = 0; ord < getInputs().size(); ord++)
            {
                ...
                var mapped = ClrDataCursorBuiltInMethod.CallAsync(implementor, ClrDataCursorBuiltInMethod.SelectAsync.MakeGenericMethod(sourceType, typeof(java.util.Map)),
                    source,
                    selector);
            ...
            return implementor.ResultAsync(physType,
                ClrDataCursorBuiltInMethod.CallAsync(implementor, ClrDataCursorBuiltInMethod.CombineQueryResultsAsync.MakeGenericMethod(typeof(object[])),
                    Expression.NewArrayInit(typeof(Func<CancellationToken, System.Threading.Tasks.ValueTask<Runtime.ClrDataCursor<java.util.Map>>>), arguments),
                    combine));
        }
