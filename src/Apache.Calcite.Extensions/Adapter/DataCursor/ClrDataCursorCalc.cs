...
namespace Apache.Calcite.Extensions.Adapter.DataCursor
{
    ...
    /// <summary>
    /// Implementation of <see cref="Calc"/> in the <see cref="ClrDataCursorConvention"/> calling convention.
    /// </summary>
    /// <remarks>
    /// Calcite fuses the filter and the projection into one anonymous <c>Enumerator</c>, because generated
    /// Java source is the only place it can put a custom enumerator. <see cref="ClrDataCursorDefaults.Calc"/> is that
    /// enumerator, written once as a cursor and called from the tree, so the plan is the same one pass over
    /// the input.
    /// </remarks>
    public class ClrDataCursorCalc : Calc, ClrDataCursorRel
    {

        /// <summary>
        /// Creates a <see cref="ClrDataCursorCalc"/>.
        /// </summary>
        /// <param name="input"></param>
        /// <param name="program"></param>
        /// <returns></returns>
        public static ClrDataCursorCalc Create(RelNode input, RexProgram program)
        {
            var cluster = input.getCluster();
            var mq = cluster.getMetadataQuery();
            var traitSet = cluster.traitSet()
                .replace(ClrDataCursorConvention.Instance)
            ...
            return new ClrDataCursorCalc(cluster, traitSet, input, program);
        }
        ...
        /// <param name="input"></param>
        /// <param name="program"></param>
        public ClrDataCursorCalc(RelOptCluster cluster, RelTraitSet traitSet, RelNode input, RexProgram program) :
        ...
        {
            return new ClrDataCursorCalc(getCluster(), traitSet, child, program);
        }
        ...
        public ClrDataCursorResult Implement(ClrDataCursorRelImplementor implementor, ClrEnumerablePrefer pref)
        {
            ...
            return implementor.Result(physType,
                Expression.Call(null, ClrDataCursorBuiltInMethod.Calc.MakeGenericMethod(inputType, outputType), result.Expression, predicate, selector));
        }
        ...
        public ClrDataCursorAsyncResult ImplementAsync(ClrDataCursorRelImplementor implementor, ClrEnumerablePrefer pref)
        {
            ...
            return implementor.ResultAsync(physType,
                ClrDataCursorBuiltInMethod.CallAsync(implementor, ClrDataCursorBuiltInMethod.CalcAsync.MakeGenericMethod(inputType, outputType), result.Expression, predicate, selector));
        }
