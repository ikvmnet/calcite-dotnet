...
namespace Apache.Calcite.Extensions.Adapter.DataCursor
{
    ...
    /// <summary>
    /// Implementation of <see cref="Window"/> in the <see cref="ClrDataCursorConvention"/> calling convention.
    /// </summary>
    /// <remarks>
    /// The loop is <see cref="ClrDataCursorDefaults.Window"/>, written once, exactly as the loop of a calc is
    /// <see cref="ClrDataCursorDefaults.Calc"/>: partitioning the input, ordering each partition, walking its rows,
    ...
    /// </remarks>
    public class ClrDataCursorWindow : Window, ClrDataCursorRel
    {
        ...
        public ClrDataCursorWindow(RelOptCluster cluster, RelTraitSet traitSet, RelNode input, java.util.List constants, RelDataType rowType, java.util.List groups) :
        ...
        {
            return new ClrDataCursorWindow(getCluster(), traitSet, (RelNode)sole(inputs), constants, getRowType(), groups);
        }

        /// <inheritdoc />
        public override Window copy(java.util.List constants)
        {
            return new ClrDataCursorWindow(getCluster(), getTraitSet(), getInput(), constants, getRowType(), groups);
        }
        ...
        Expression ImplementGroup(
            ClrDataCursorRelImplementor implementor,
            ...
            out ClrPhysType outputPhysType)
        {
            ...
            return Expression.Call(null,
                ClrDataCursorBuiltInMethod.Window.MakeGenericMethod(sourceType, keyType, accType, outputType),
                source,
            ...
            out ClrPhysType outputPhysType)
        {
            ...
            return ClrDataCursorBuiltInMethod.CallAsync(implementor, ClrDataCursorBuiltInMethod.WindowAsync.MakeGenericMethod(sourceType, keyType, accType, outputType),
                source,
                (Expression?)partitionSelector ?? Expression.Constant(null, typeof(Func<,>).MakeGenericType(sourceType, keyType)),
                comparator,
                Expression.Constant(group.exclude, typeof(RexWindowExclusion)),
