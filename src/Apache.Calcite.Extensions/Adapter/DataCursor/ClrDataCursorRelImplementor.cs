...
namespace Apache.Calcite.Extensions.Adapter.DataCursor
{
    ...
    /// <summary>
    /// Turns a tree of <see cref="ClrDataCursorRel"/> into the factory that opens it.
    /// </summary>
    /// <remarks>
    ...
    /// <see cref="VisitChild"/> calls only <see cref="ClrDataCursorRel.Implement"/> and
    /// <see cref="VisitChildAsync"/> only <see cref="ClrDataCursorRel.ImplementAsync"/>, so a body always
    ...
    /// walks the tree once through each hierarchy and hands back a <see cref="ClrDataCursorFactory"/>
    ...
    /// it by <see cref="ClrDataCursorBuiltInMethod.CallAsync"/>. A deferred open built by
    ...
    /// </remarks>
    public class ClrDataCursorRelImplementor : IClrRelImplementor
    {
        ...
        public ClrDataCursorRelImplementor(RexBuilder rexBuilder, java.util.Map internalParameters) :
        ...
        /// <remarks>
        /// What a node's <see cref="ClrDataCursorRel.Implement"/> calls for an eager input, and it calls the
        /// input's <see cref="ClrDataCursorRel.Implement"/> in turn. The synchronous hierarchy is closed: every
        /// call in it reaches a synchronous body and answers a synchronous open.
        /// </remarks>
        public ClrDataCursorResult VisitChild(ClrDataCursorRel? parent, int ordinal, ClrDataCursorRel child, ClrEnumerablePrefer prefer)
        {
        ...
        /// <returns>The input's open, physical type and row format.</returns>
        /// <remarks>
        /// The awaiting counterpart, and the one place <see cref="ClrDataCursorRel.ImplementAsync"/> is
        /// called from a node.
        /// </remarks>
        public ClrDataCursorAsyncResult VisitChildAsync(ClrDataCursorRel? parent, int ordinal, ClrDataCursorRel child, ClrEnumerablePrefer prefer)
        {
            ArgumentNullException.ThrowIfNull(child);
        ...
        /// <remarks>
        /// A node whose only real body is the awaiting one writes <see cref="ClrDataCursorRel.Implement"/>
        ...
        /// <see cref="ClrDataCursors.Block{T}"/> can suppress the synchronization context before it starts.
        /// </remarks>
        public ClrDataCursorResult Pulled(ClrDataCursorAsyncResult result)
        {
            ArgumentNullException.ThrowIfNull(result);

            return new ClrDataCursorResult(
                Expression.Call(null, ClrDataCursorBuiltInMethod.Block.MakeGenericMethod(result.PhysType.RowType), OpenerAsync(result)),
                result.PhysType,
                result.Format);
        }
        ...
        /// <see cref="ClrDataCursorRel.ImplementAsync"/> is, and what a node writes when its awaiting body
        /// has nothing to await on the way to its cursor. It allocates nothing.
        /// </remarks>
        public ClrDataCursorAsyncResult Awaited(ClrDataCursorResult result)
        {
            ArgumentNullException.ThrowIfNull(result);

            return new ClrDataCursorAsyncResult(
                Expression.Call(null, ClrDataCursorBuiltInMethod.Completed.MakeGenericMethod(result.PhysType.RowType), result.Expression),
                result.PhysType,
                result.Format);
        }
        ...
        /// <returns>A <c>Func&lt;ClrDataCursor&lt;TRow&gt;&gt;</c> that runs the open when called.</returns>
        /// <remarks>
        ...
        /// </remarks>
        public LambdaExpression Opener(ClrDataCursorResult result)
        {
            ArgumentNullException.ThrowIfNull(result);
        ...
        /// </summary>
        /// <param name="result">The input's open.</param>
        /// <returns>A <c>Func&lt;CancellationToken, ValueTask&lt;ClrDataCursor&lt;TRow&gt;&gt;&gt;</c> that
        ...
        /// </remarks>
        public LambdaExpression OpenerAsync(ClrDataCursorAsyncResult result)
        {
            ArgumentNullException.ThrowIfNull(result);
        ...
        /// <summary>
        /// Implements a whole plan as a <see cref="ClrDataCursorFactory"/>, which opens it against the
        ...
        /// </remarks>
        public ClrDataCursorFactory ImplementRoot(ClrDataCursorRel rootRel, ClrEnumerablePrefer prefer)
        {
            ArgumentNullException.ThrowIfNull(rootRel);

            ClrDataCursorResult pulled;
            ClrDataCursorAsyncResult awaited;
                ...
                && rootRel.getRowType().getFieldCount() == 1)
            {
                ...
                pulled = new ClrDataCursorResult(
                    Expression.Call(null, ClrDataCursorBuiltInMethod.Slice0.MakeGenericMethod(rowType), pulled.Expression),
                    pulled.PhysType,
                    JavaRowFormat.SCALAR);

                awaited = new ClrDataCursorAsyncResult(
                    ClrDataCursorBuiltInMethod.CallAsync(this, ClrDataCursorBuiltInMethod.Slice0Async.MakeGenericMethod(rowType), awaited.Expression),
                    awaited.PhysType,
                    JavaRowFormat.SCALAR);
            }
            ...
            var open = Expression.Lambda<Func<DataContext, ClrDataCursor>>(
                Expression.Convert(pulled.Expression, typeof(ClrDataCursor)),
                Root);

            var openAsync = Expression.Lambda<Func<DataContext, CancellationToken, ValueTask<ClrDataCursor>>>(
                Expression.Call(null, ClrDataCursorBuiltInMethod.Untyped.MakeGenericMethod(rowType), awaited.Expression),
                Root,
                CancellationToken);

            return new ClrDataCursorFactory(open, openAsync, ElementType(rootRel, prefer));
        }

        /// <summary>
        /// Returns what one row of the plan is, which is the same answer whichever way it is opened.
        /// </summary>
        Type ElementType(ClrDataCursorRel rel, ClrEnumerablePrefer prefer)
        {
        ...
        /// <summary>
        /// Creates the result a node's <see cref="ClrDataCursorRel.Implement"/> returns.
        /// </summary>
        /// <param name="physType">How the rows are represented.</param>
        /// <param name="expression">The open, whose value must be a <c>ClrDataCursor&lt;TRow&gt;</c> of the
        /// physical row type.</param>
        /// <returns></returns>
        /// <exception cref="java.lang.IllegalStateException">The open is not that.</exception>
        public ClrDataCursorResult Result(ClrPhysType physType, Expression expression)
        {
        ...
        /// <summary>
        /// Creates the result a node's <see cref="ClrDataCursorRel.ImplementAsync"/> returns.
        /// </summary>
        /// <param name="physType">How the rows are represented.</param>
        /// <param name="expression">The open, whose value must be a
        /// <c>ValueTask&lt;ClrDataCursor&lt;TRow&gt;&gt;</c> of the physical row type.</param>
        /// <returns></returns>
        /// <exception cref="java.lang.IllegalStateException">The open is not that.</exception>
        /// <remarks>
        ...
        /// </remarks>
        public ClrDataCursorAsyncResult ResultAsync(ClrPhysType physType, Expression expression)
        {
            ...
            if (cursorDefinition != typeof(ClrDataCursor<>))
                throw new java.lang.IllegalStateException($"{Node()} handed up a {expression.Type} where an awaiting open of {physType.RowType} was wanted.");
            ...
            return new ClrDataCursorAsyncResult(expression, physType, physType.Format);
        }
        ...
        static string Node()
        {
            ...
            for (int i = 1; i < trace.FrameCount; i++)
            {
                ...
                if (type != null && type != typeof(ClrDataCursorRelImplementor))
                    return type.Name;
            }
