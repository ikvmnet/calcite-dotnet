...
namespace Apache.Calcite.Extensions.Adapter.DataCursor
{
    ...
    /// <summary>
    /// Rule that converts a <see cref="LogicalJoin"/> to a <see cref="ClrDataCursorHashJoin"/> or, where
    /// there is no equality to build a lookup on, to a <see cref="ClrDataCursorNestedLoopJoin"/>.
    /// </summary>
    /// <remarks>
    ...
    /// </remarks>
    public class ClrDataCursorJoinRule : ConverterRule
    {

        /// <summary>
        /// Creates a <see cref="ClrDataCursorJoinRule"/>.
        /// </summary>
        /// <returns></returns>
        public static ClrDataCursorJoinRule Create()
        {
            return (ClrDataCursorJoinRule)Config.INSTANCE
                .withConversion((java.lang.Class)typeof(LogicalJoin), Convention.NONE, ClrDataCursorConvention.Instance, "ClrDataCursorJoinRule")
                .withRuleFactory(new DelegateFunction<Config, ClrDataCursorJoinRule>(c => new ClrDataCursorJoinRule(c)))
                .toRule(typeof(ClrDataCursorJoinRule));
        }
        ...
        public ClrDataCursorJoinRule(Config config) :
            base(config)
        {
        ...
        public override RelNode? convert(RelNode rel)
        {
            ...
            if (hasEquiKeys)
            {
                ...
                return ClrDataCursorHashJoin.Create(left, right, condition, join.getVariablesSet(), join.getJoinType());
            }

            return ClrDataCursorNestedLoopJoin.Create(left, right, join.getCondition(), join.getVariablesSet(), join.getJoinType());
        }
