...
namespace Apache.Calcite.Extensions.Adapter.DataCursor
{
    ...
    /// <summary>
    /// Implementation of <see cref="TableScan"/> in the <see cref="ClrDataCursorConvention"/> calling
    /// convention.
    /// </summary>
    /// <remarks>
    ...
    /// </remarks>
    public class ClrDataCursorTableScan : TableScan, ClrDataCursorRel
    {

        /// <summary>
        /// Creates a <see cref="ClrDataCursorTableScan"/>.
        /// </summary>
        /// <param name="cluster"></param>
        /// <param name="relOptTable"></param>
        /// <returns></returns>
        public static ClrDataCursorTableScan Create(RelOptCluster cluster, RelOptTable relOptTable)
        {
            ...
            return new ClrDataCursorTableScan(cluster, traitSet, relOptTable, elementType);
        }
        ...
        public ClrDataCursorTableScan(RelOptCluster cluster, RelTraitSet traitSet, RelOptTable table, java.lang.Class elementType) :
            base(cluster, traitSet, com.google.common.collect.ImmutableList.of(), table)
        {
            ...
            if (CanHandle(table) == false)
                throw new java.lang.AssertionError($"ClrDataCursorTableScan can't implement {table}, see ClrDataCursorTableScan.CanHandle");
        ...
        {
            return new ClrDataCursorTableScan(getCluster(), traitSet, table, elementType);
        }
        ...
        /// is opened over it.
        /// </remarks>
        Expression ClrSource(ClrDataCursorRelImplementor implementor)
        {
            var unwrapped = (Table)table.unwrap(typeof(Table));
            ...
            return Expression.Call(null, ClrDataCursorBuiltInMethod.AsCursor.MakeGenericMethod(element), sequence);
        }
        ...
        /// </remarks>
        Expression ClrSourceAsync(ClrDataCursorRelImplementor implementor)
        {
            var unwrapped = (Table)table.unwrap(typeof(Table));
            var element = ClrTypes.FromClass(elementType);
            ...
            return ClrDataCursorBuiltInMethod.CallAsync(implementor, ClrDataCursorBuiltInMethod.AsCursorAsync.MakeGenericMethod(element), sequence);
        }
        ...
        /// <returns></returns>
        Expression ToRows(ClrDataCursorRelImplementor implementor, ClrPhysType physType, Expression source, bool native)
        {
            var element = ClrTypes.FromClass(elementType);
            ...
            if (physType.Format == JavaRowFormat.SCALAR
                ...
                return Expression.Call(null,
                    ClrDataCursorBuiltInMethod.Slice0.MakeGenericMethod(physType.RowType),
                    Source(element));
            ...
            return Expression.Call(null, ClrDataCursorBuiltInMethod.Select.MakeGenericMethod(element, rowType), Source(element), selector);
        }
        ...
        /// <param name="native">Whether <paramref name="source"/> is already a cursor.</param>
        /// <returns></returns>
        Expression ToRowsAsync(ClrDataCursorRelImplementor implementor, ClrPhysType physType, Expression source, bool native)
        {
            var element = ClrTypes.FromClass(elementType);
            ...
            if (physType.Format == JavaRowFormat.SCALAR
                ...
                return ClrDataCursorBuiltInMethod.CallAsync(implementor,
                    ClrDataCursorBuiltInMethod.Slice0Async.MakeGenericMethod(physType.RowType),
                    Source(element));
