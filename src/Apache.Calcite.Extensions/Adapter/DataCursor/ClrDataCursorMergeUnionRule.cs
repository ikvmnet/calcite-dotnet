...
namespace Apache.Calcite.Extensions.Adapter.DataCursor
{
    ...
    /// <see cref="ClrDataCursorMergeUnion"/>.
    /// </summary>
    /// <remarks>
    ...
    /// </remarks>
    public class ClrDataCursorMergeUnionRule : RelRule
    {

        /// <summary>
        /// Creates a <see cref="ClrDataCursorMergeUnionRule"/>.
        /// </summary>
        /// <returns></returns>
        public static ClrDataCursorMergeUnionRule Create()
        {
            var config = org.apache.calcite.adapter.enumerable.EnumerableMergeUnionRule.Config.DEFAULT_CONFIG
                .withDescription("ClrDataCursorMergeUnionRule");

            return new ClrDataCursorMergeUnionRule(config);
        }
        ...
        public ClrDataCursorMergeUnionRule(RelRule.Config config) :
            base(config)
        {
        ...
        public override void onMatch(RelOptRuleCall call)
        {
            ...
            for (int i = 0; i < union.getInputs().size(); i++)
            {
                ...
                inputs.add(convert(call.getPlanner(), newInput, newInput.getTraitSet().replace(ClrDataCursorConvention.Instance)));
            }

            RelNode result = ClrDataCursorMergeUnion.Create(sort.getCollation(), inputs, union.all);

            // the merge union's rows are already in order, so an offset or a limit is all that is left of the
            // sort
            if (sort.offset != null || sort.fetch != null)
                result = ClrDataCursorLimit.Create(result, sort.offset, sort.fetch);
