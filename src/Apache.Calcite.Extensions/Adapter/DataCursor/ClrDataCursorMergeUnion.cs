...
namespace Apache.Calcite.Extensions.Adapter.DataCursor
{
    ...
    /// <summary>
    /// Implementation of a union in the <see cref="ClrDataCursorConvention"/> calling convention, by merging
    ...
    /// </remarks>
    public class ClrDataCursorMergeUnion : ClrDataCursorUnion
    {

        /// <summary>
        /// Creates a <see cref="ClrDataCursorMergeUnion"/>.
        /// </summary>
        /// <param name="collation"></param>
        /// <param name="inputs"></param>
        /// <param name="all"></param>
        /// <returns></returns>
        public static ClrDataCursorMergeUnion Create(RelCollation collation, java.util.List inputs, bool all)
        {
            ...
            return new ClrDataCursorMergeUnion(cluster, traitSet, inputs, all);
        }
        ...
        /// <param name="inputs"></param>
        /// <param name="all"></param>
        public ClrDataCursorMergeUnion(RelOptCluster cluster, RelTraitSet traitSet, java.util.List inputs, bool all) :
            base(cluster, traitSet, inputs, all)
        {
            ...
            if (collations.isEmpty() || ((RelCollation)collations.get(0)).getFieldCollations().isEmpty())
                throw new java.lang.IllegalArgumentException("ClrDataCursorMergeUnion with no collation");
                ...
                for (int j = 0; j < collations.size(); j++)
                {
                    ...
                    if (satisfied == false)
                        throw new java.lang.IllegalArgumentException(
                            $"ClrDataCursorMergeUnion input does not satisfy collation. ClrDataCursorMergeUnion collation: {collation}. Input collations: {inputCollations}. Input: {inputs.get(i)}");
                }
            }
        }
        ...
        {
            return new ClrDataCursorMergeUnion(getCluster(), traitSet, inputs, all);
        }

        /// <inheritdoc />
        public override ClrDataCursorResult Implement(ClrDataCursorRelImplementor implementor, ClrEnumerablePrefer pref)
        {
            ...
            if (collation == null || collation.getFieldCollations().isEmpty())
                throw new java.lang.IllegalStateException("ClrDataCursorMergeUnion with no collation");
            ...
            body.Add(
                Expression.Call(null,
                    ClrDataCursorBuiltInMethod.MergeUnion.MakeGenericMethod(rowType, sortKeySelector.ReturnType),
                    sources,
                    sortKeySelector,
                    sortComparator,
        ...
        /// <inheritdoc />
        public override ClrDataCursorAsyncResult ImplementAsync(ClrDataCursorRelImplementor implementor, ClrEnumerablePrefer pref)
        {
            var physType = ClrPhysTypeImpl.Of(implementor.TypeFactory, getRowType(), pref.Prefer(JavaRowFormat.CUSTOM));
            var rowType = physType.RowType;
            ...
            body.Add(
                ClrDataCursorBuiltInMethod.CallAsync(implementor, ClrDataCursorBuiltInMethod.MergeUnionAsync.MakeGenericMethod(rowType, sortKeySelector.ReturnType),
                    sources,
                    sortKeySelector,
                    sortComparator,
                    Expression.Constant(all),
