...
namespace Apache.Calcite.Extensions.Adapter.DataCursor
{
    ...
    /// <see cref="ClrDataCursorBatchNestedLoopJoin"/>.
    /// </summary>
    /// <remarks>
    ...
    /// </remarks>
    public class ClrDataCursorBatchNestedLoopJoinRule : RelRule
    {

        /// <summary>
        /// Creates a <see cref="ClrDataCursorBatchNestedLoopJoinRule"/>, with Calcite's default batch size.
        /// </summary>
        /// <returns></returns>
        public static ClrDataCursorBatchNestedLoopJoinRule Create()
        {
            return Create(org.apache.calcite.adapter.enumerable.EnumerableBatchNestedLoopJoinRule.Config.DEFAULT.batchSize());
        }

        /// <summary>
        /// Creates a <see cref="ClrDataCursorBatchNestedLoopJoinRule"/>.
        /// </summary>
        /// <param name="batchSize">How many left rows one pass of the right input serves.</param>
        /// <returns></returns>
        public static ClrDataCursorBatchNestedLoopJoinRule Create(int batchSize)
        {
            var config = org.apache.calcite.adapter.enumerable.EnumerableBatchNestedLoopJoinRule.Config.DEFAULT
                .withBatchSize(batchSize)
                .withDescription($"ClrDataCursorBatchNestedLoopJoinRule({batchSize})");

            return new ClrDataCursorBatchNestedLoopJoinRule((RelRule.Config)config, batchSize);
        }
        ...
        public ClrDataCursorBatchNestedLoopJoinRule(RelRule.Config config, int batchSize) :
            base(config)
        {
        ...
        public override void onMatch(RelOptRuleCall call)
        {
            ...
            call.transformTo(
                ClrDataCursorBatchNestedLoopJoin.Create(
                    convert(call.getPlanner(), join.getLeft(), join.getLeft().getTraitSet().replace(ClrDataCursorConvention.Instance)),
                    convert(call.getPlanner(), right, right.getTraitSet().replace(ClrDataCursorConvention.Instance)),
                    join.getCondition(),
