...
one), the hash, merge and nested loop joins, sort, limit and union — exist, and every query the
differential suite runs through them answers
...
- **The nodes not yet ported** — *large, and each is a transcription*. Aggregate and sorted aggregate,
  the batch nested loop and ASOF joins, correlate and conditional correlate,
